...
void demonstrate_pipe_capacity() {
    ...
    switch (cpid = fork()) {
        ...
        case 0:  /* child process - stays idle initially */
            close(fd[1]); // Close write end
            
            printf("Child: Waiting for parent to fill the pipe buffer...\n");
            printf("Child: I will NOT read from the pipe initially\n");
            printf("Child: This allows us to see the pipe buffer limit\n");
            
            // Child sleeps for 5 seconds to let parent fill pipe completely
            sleep(5);
            
            printf("Child: Now starting to read and drain the pipe...\n");
            char read_data;
            int read_count = 0;
            
            while (read(fd[0], &read_data, 1) > 0) {
                read_count++;
                if (read_count % 8192 == 0) {  // Report every 8KB
                    printf("Child: Drained %d bytes from pipe\n", read_count);
                }
            }
            
            printf("Child: Total bytes drained from pipe: %d\n", read_count);
            close(fd[0]);
            exit(0);
        ...
        default: /* parent process - writer */
            ...
            printf("Parent: Attempting to fill pipe buffer completely...\n");
            printf("Parent: Writing will block when buffer is full\n");
            
            // Keep original blocking behavior - this will block when pipe is full
            while (1) {
                ssize_t result = write(fd[1], &data, 1);
                
                if (result == 1) {
                    write_count++;
                    // Report less frequently to reduce output spam
                    if (write_count % 8192 == 0) {  // Report every 8KB
                        printf("Parent: Written %d bytes (%.1f KB)\n", 
                               write_count, write_count / 1024.0);
                    }
                } else {
                    printf("Parent: Write failed after %d bytes\n", write_count);
                    perror("write");
                    break;
                }
                
                // Safety check to prevent truly infinite loop
                if (write_count >= 1000000) {  // 1MB limit
                    printf("Parent: Reached safety limit of 1MB, stopping\n");
                    break;
                }
            }
            
            printf("Parent: Pipe buffer filled with %d bytes (%.1f KB)\n", 
                   write_count, write_count / 1024.0);
            printf("Parent: This shows the approximate pipe buffer capacity\n");
            
            close(fd[1]);
            wait(NULL);
            printf("Parent: Child has finished draining the pipe\n");
    }
